...
- [ ] Each candidate semantic has a framework-neutral definition and expected
      fallback.
- [ ] Host mappings cite current primary documentation or current source.
- [ ] The report separates catalog visibility, implicit selection, explicit
      selection, router reachability, and context cost.
- [ ] One disposable consumer experiment exercises supported and unsupported
      behavior.
- [ ] Static fixtures are not represented as runtime/model proof.
- [ ] The report identifies whether production work belongs in registry
      metadata, installation adapters, plugin packaging, or a separate product.
- [ ] The maintainer records go, revise, or no-go and selects the first
