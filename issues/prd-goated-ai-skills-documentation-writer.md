...
Draft, created 2026-05-27. Ready for maintainer review before `issues/049-implement-documentation-writer-manuals-first.md` starts.

## Problem

GOATED AI Skills already includes workflows for PRDs, implementation planning, review, verification, and documentation drift sync. It does not yet have a dedicated skill for intentionally authoring durable user-facing documentation before or alongside product delivery.
...
- Maintainers and contributors who need a clear v1 boundary before creating `skills/engineering/documentation-writer/`.
- Future agents implementing `issues/049-implement-documentation-writer-manuals-first.md`.
...
- Add a future `documentation-writer` engineering skill for planned, source-grounded documentation authoring.
...
- Do not implement `documentation-writer` in this PRD issue.
- Do not create `skills/engineering/documentation-writer/` until `issues/049-implement-documentation-writer-manuals-first.md`.
...
- Do not list `documentation-writer` as implemented in `README.md`, `docs/how-to-use.md`, or category READMEs before the implementation issue is complete.
...
| --- | --- | --- |
| Create a manuals-first authoring skill | Must | Future `documentation-writer` v1 should create or substantially revise user manuals, operator guides, onboarding guides, and durable product documentation. |
...
| Preserve GOATED skill shape | Must | The future skill must follow the lean schema, stay self-contained after installation, and use support files when templates, examples, checklists, or QA rubrics would bloat `SKILL.md`. |
...
The future skill should create an AI companion doc when the user asks for it, when a project has repeated agent work, or when the documentation set would benefit from a model-oriented read path. For small documents, the skill may instead report that a separate AI companion doc is unnecessary and explain why.

## Implementation Notes

- Implement the future skill at `skills/engineering/documentation-writer/SKILL.md` only in `issues/049-implement-documentation-writer-manuals-first.md`.
...
- Keep the future `SKILL.md` lean. Put detailed document templates, outline examples, AI companion doc rubrics, and manual QA checklists in directly linked `references/` files if they materially improve the skill.
- Recommended soft dependencies for the future skill include `session-start-progressive-disclosure`, `grill-with-docs`, `write-a-prd`, `doc-sync`, and `verification-before-completion`.
- The future workflow should start by confirming the documentation purpose, audience, doc type, source evidence, location convention, and freshness expectations.
...
- [ ] `skills/engineering/documentation-writer/SKILL.md` is created only by `issues/049-implement-documentation-writer-manuals-first.md` after this PRD is reviewed and approved.
- [ ] The implemented skill follows the GOATED lean schema and remains self-contained after installation.
- [ ] The implemented skill is scoped to manuals-first durable documentation: user manuals, operator guides, onboarding guides, durable product docs, and optional separate AI-optimized companion docs.
- [ ] The workflow gathers source evidence before writing and records assumptions, gaps, and unresolved product questions.
- [ ] The workflow chooses documentation locations from target-project conventions before falling back to a simple durable Markdown default.
- [ ] The workflow distinguishes planned documentation authoring from `doc-sync` drift handling.
- [ ] The output contract covers created or updated docs, source evidence, audience, scope, verification, `doc-sync` relationship, and remaining gaps.
- [ ] Delegation supports bounded source summarization, outline review, audience fit review, AI companion doc review, and documentation QA.
- [ ] Guardrails prevent private data, credentials, raw private transcripts, unverified product claims, and documentation website or static-site generation in v1.
- [ ] `README.md`, `docs/how-to-use.md`, and `skills/engineering/README.md` list `documentation-writer` only after issue 049 implements the skill.

## Testing And Verification

- Manually review `skills/engineering/documentation-writer/SKILL.md` against the lean schema after issue 049 creates it.
- Check that the future skill can author at least one human-facing manual or guide and can decide whether a separate AI-optimized companion doc is warranted.
...
- Use manual Markdown review as the expected proof for this Markdown-only PRD and future docs-only implementation, unless later tooling is added.

## Risks And Mitigations

| Risk | Impact | Mitigation |
| --- | --- | --- |
| The skill becomes a clone of `doc-sync` | Agents blur planned authoring and drift cleanup | Keep trigger, output, and non-goal boundaries explicit in the PRD and future `SKILL.md`. |
...
| The future `SKILL.md` becomes too large | Agents load too much context before acting | Move templates, examples, and QA checklists into linked `references/` files. |
...
| --- | --- | --- |
| Should AI-optimized companion docs have a default filename convention, such as `docs/agents/<topic>-manual.md` or `docs/agents/<topic>-context.md`? | Maintainer | Issue 049 implementation, unless the skill records a convention-discovery default instead. |
| Should v1 include reusable templates for manuals, operator guides, onboarding guides, product docs, and AI companion docs under `references/`? | Maintainer or issue 049 implementer | Issue 049 implementation. |
| Should runbooks and troubleshooting guides be named explicitly as operator-guide variants? | Maintainer or issue 049 implementer | Issue 049 implementation. |

## Follow-Up Issue

The implementation issue is `issues/049-implement-documentation-writer-manuals-first.md`.

Issue 049 remains blocked until this PRD exists and receives maintainer approval. Issue 048 creates the draft PRD but does not by itself satisfy that approval gate.
...
- `issues/048-draft-documentation-writer-prd.md` - implementation contract for this PRD draft.
- `issues/049-implement-documentation-writer-manuals-first.md` - planned follow-up implementation issue.
