...
- [x] `skills/agent-workflows/agent-instructions-integrator/SKILL.md` exists and follows the lean schema.
- [x] The workflow detects or accepts the chosen agent framework before editing instructions.
- [x] The workflow supports Codex, Claude Code, Hermes, OpenCode, and generic agent-framework routing at a docs-first level.
- [x] The output contract includes the selected instruction artifact, installed skill routing, target-project artifact routing, and any unresolved framework assumptions.
- [x] The guardrails prohibit pasting the full GOATED workflow into every target project.
- [x] The skill does not assume `AGENT.md` is universal.
