...
| `issues/*.md` excluding `issues/archive/` and PRDs | Future implementation or acceptance handoffs after V1. | Active post-V1 handoffs include `051` and `052`. | Read the specific active issue before implementing or reviewing that slice. |
...
- Date: 2026-05-31
- Updated by: Codex
- Evidence used: prior 2026-05-23 matrix evidence; active post-V1 issues `051` and `052`; archived issue `050`; documentation-cleanup implementation; targeted catalog and issue-state scans with `rg -n`; `git status --short`; issue-file heading review.
