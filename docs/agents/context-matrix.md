...
| `issues/archive/*.md` | Completed scaffold, skill implementation, and follow-up upgrade handoffs. | Archived. | As of 2026-05-21, issues `001` through `020` and `022` through `026` are archived. Use the specific archived issue to understand why an existing artifact was created or upgraded. |
| `issues/*.md` excluding `issues/archive/` and PRDs | Planned future V1 implementation slices and final acceptance pass. | Active issue handoffs. | As of 2026-05-21, active handoffs are `021` and `027` through `043`; read the current issue before implementing or reviewing that slice. |
