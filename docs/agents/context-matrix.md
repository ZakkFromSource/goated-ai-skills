...
| `rg --files` | Discover repo files without bulk-reading them. | At the start of mapping, source routing, or targeted scans. | Ran on 2026-05-22 during `writing-plans` implementation; showed root docs, active/archived issues, docs, and 23 implemented skill folders. |
...
| `issues/archive/*.md` | Completed scaffold, skill implementation, and follow-up upgrade handoffs. | Archived. | As of 2026-05-22, issues `001` through `020`, `022` through `026`, `029`, and `033` through `038` are archived. Use the specific archived issue to understand why an existing artifact was created or upgraded. |
| `issues/*.md` excluding `issues/archive/` and PRDs | Planned future V1 implementation slices and final acceptance pass. | Active issue handoffs. | As of 2026-05-22, active handoffs are `021`, `027`, `028`, `030` through `032`, and `039` through `043`; read the current issue before implementing or reviewing that slice. |
...
- Date: 2026-05-22
- Updated by: Codex
- Evidence used: installed `session-start-progressive-disclosure`, `context-matrix-map`, `project-standards-calibration`, and `doc-sync`; `AGENT.md`; `CONTEXT.md`; `README.md`; `docs/install.md`; `docs/agents/project-standards.md`; category READMEs under `skills/`; implemented skill inventory under `skills/`; `docs/adr/README.md`; `docs/adr/0001-v1-runtime-bootstrap-and-adapter-automation.md`; `.gitignore`; `.out-of-scope/future-automation-and-validation.md`; V1 PRDs; active issues `021`, `027`, `028`, `030` through `032`, and `039` through `043`; archived issues `001` through `020`, `022` through `026`, `029`, and `033` through `038`; `rg --files`; targeted issue and stale-wording scans with `rg -n`; `git rev-parse --show-toplevel`; `git status --short`; manifest and test discovery `rg` commands.
