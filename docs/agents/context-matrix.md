...
| `issues/archive/*.md` | Completed scaffold, skill implementation, and follow-up upgrade handoffs. | Archived. | Use the specific archived issue to understand why an existing artifact was created or upgraded. |
| `issues/*.md` excluding `issues/archive/` and PRDs | Planned future V1 implementation slices and final acceptance pass. | Active issue handoffs. | Read the current issue before implementing or reviewing that slice. |
