...
| Use the lean skill schema for implemented `SKILL.md` files. | review-enforced | `skills/README.md`, `issues/prd-goated-ai-skills-v1-public-core.md`, implemented skill inventory | Required frontmatter includes top-level `name`, top-level `description`, and `metadata.goated-category`. `description` is discovery-only; body sections own trigger patterns, output behavior, dependencies, and guardrails. |
...
| Completed implementation issues move under `issues/archive/` only after acceptance criteria are checked and any required user, maintainer, PR, or project-defined review is complete. | review-enforced | `CONTEXT.md`; archived issues `001` through `052`; active post-V1 issues `053`, `054`, `055`, `056`, `057`, `058`, and `059` | High |
| Docs-only changes currently rely on manual markdown review, while skill helper scripts need targeted command checks. | review-enforced | No project-wide manifest, CI, formatter, linter, or test config found in this pass; `documentation-cleanup` adds a read-only helper script | High |
...
- Date: 2026-06-11
- Updated by: Codex
- Evidence used: prior 2026-05-23 standards evidence; active post-V1 issues `053`, `054`, `055`, `056`, `057`, `058`, and `059`; archived issues `050`, `051`, and `052`; documentation-cleanup implementation; targeted catalog and issue-state scans with `rg -n`; `git status --short`; issue-file heading review.
